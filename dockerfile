...
ENV        NVIDIA_HEADERS_VERSION=12.1.14.0
...
#         mkdir -p ${DIR} && \
#         cd ${DIR} && \
#         curl -sLO https://github.com/pkuvcl/xavs2/archive/refs/tags/${XAVS2_VERSION}.tar.gz && \
#         tar -xz --strip-components=1 -f ${XAVS2_VERSION}.tar.gz && \
#         ./configure --prefix="${PREFIX}" --enable-shared  && \
#         make && \
#         make install && \
#         rm -rf ${DIR}
...
RUN  \
        ...
        curl -sLO https://ffmpeg.org/releases/ffmpeg-${FFMPEG_VERSION}.tar.xz && \
        tar -x --strip-components=1 -f ffmpeg-${FFMPEG_VERSION}.tar.xz && \
...
COPY --from=build /usr/lib/x86_64-linux-gnu/libaom.so  /usr/local/lib/
COPY --from=build /usr/lib/x86_64-linux-gnu/libaom.so.0  /usr/local/lib/
